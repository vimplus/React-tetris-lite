...
class Grid extends Component {
    ...
    render() {
        ...
        /*return (
            <canvas ref="canvas" width={this.props.cols * TileWidth} height={this.props.rows * TileHeight} />
        )*/
    }
    componentDidUpdate() {
        ...
        if (active) {
            ...
            for (let row = 0; row < active.height(); row++) {
                for (let col = 0; col < active.width(); col++) {
                    tile = active.getTile(row, col);
                    if (tile) {
                        context.fillStyle = tile.color;
                        context.fillRect( (active.col + col) * TileWidth, (active.row + row) * TileHeight, TileWidth - TilePadding, TileHeight - TilePadding);
                    }
                }
            }
        }
